...
class ExecutionBot(Management):
    ...
    def task_complete(self):
        ...
        self.store.close()
        # #
        self.logger.info(f'Bot done and connection is closed!')
        sys.exit()
        os._exit(1)
    ...
    def aggressive_orders(self, qty, action, exec_t=2, log=False):
        ...
        if action == 'buy':
            cost_qty *= -1.
        if log:
            self.save_record('AggressiveNoSlice', sym, action, qty_target, 1, min(time.time() - t_start, exec_t),
                             qty_target - qty, cost_qty, penalty, benchmark_price, benchmark_vwap)
    ...
    def save_record(self, exec_algo, sym, action, qty, slices, time, qty_exec, tc, penalty, bp, vwap):
        ...
        record = {'AgentID': self.bot_id,
                  ...
                  'Action': action,
                  'ExecTargetQty': int(qty),
                  'ExecSlices': int(slices),
                  'ExecTime(secs)': int(time),
                  'ExecActualQty': int(qty_exec),
                  'TransactionCost': float(tc),
                  'Penalty': float(penalty),
                  'BenchmarkPrice': float(bp),
                  'BenchmarkVWAP': float(vwap)
                  }

        record = pd.DataFrame(record, index=[0])

        self.store('{`ExecutionBot upsert x}', record)
    ...
    def customPOV_orders(self, qty, action, exec_t=10, p_rate=0.25, sub_window=3.0):
        ...
        while qty_target > 0 and time.time() - time_s < exec_t:
            ...
            y = 0
            lvl = 1
            while size < qt and len(book_levels) > 0:
                level = 'L'+str(lvl)
        ...
        self.save_record('POV', sym, action, qty, i_slice - 1, sub_window * (i_slice - 1),
                         qty - qty_target, cost_qty, penalty, benchmark_price, benchmark_vwap)
...
if __name__ == "__main__":
    ...
    qty_slice = 0.1
    for _ in range(1):
        # exec_bot.aggressive_orders(args.size, args.action, 5, log=True)
        exec_bot.customPOV_orders(args.size, args.action, exec_t=args.maxtime, sub_window=time_slice * args.maxtime)
